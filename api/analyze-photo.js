...
1단계: 사진에서 각 문항 번호(01, 02, 03...) 영역을 찾아라. 그 번호 바로 옆/위에 손으로 그린 색깔 표시만 rawObservations에 나열하라. 문제 본문 안(그림, 도형, 풀이 과정, 선택지, 빈칸 등)에 있는 표시는 완전히 무시하라. 오직 문항 번호 옆 표시만 관찰하라.
${modeInstruction}
...
- ⚠️ 채점 표시 위치 규칙 (가장 중요):
  · 채점 표시는 반드시 "문항 번호(01, 02, 03...)" 바로 옆이나 위에 있는 것만 인정한다
  · 문제 본문 안(그림, 도형, 선택지, 풀이 과정, 빈칸 등)에 있는 원/동그라미/표시는 채점 표시가 아니다
  · 문제 내용에 포함된 빨간 원, 문자 주변 원, 그림 위의 원 등은 모두 무시하라
  · 오직 문항 번호 영역(번호판/숫자 옆)에 있는 손으로 그린 표시만 채점 기호로 인식하라
