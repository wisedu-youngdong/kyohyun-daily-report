...
**⚠️ Step 4는 예외가 없다. 본문 텍스트, 수식, 그림, 보기 주변의 원은 Step 4에서 무조건 무시된다.**

# ⚠️ 정답/오답 최종 판정 규칙 (예외 없음)

정답 조건 (둘 중 하나):
1. 문제 번호(숫자) 자체를 감싸는 빨간 원이 있다
2. 문제 번호 + 문제 내용 전체를 크게 감싸는 빨간 원/타원이 있다

오답 조건 (다음 중 하나라도 해당):
1. 문항 번호 위/옆에 빨간 빗금(/) 또는 X가 있다 → 무조건 오답
2. 정답 조건(위 1, 2번)에 해당하는 원이 없다 → 오답
3. 빗금이 있는 문항은 같은 문항 본문 안에 원이 있어도 오답이다

핵심: "정답임을 증명하는 원이 없으면 오답"이다.
의심스러우면 오답으로 처리하라.
