...
- **Step 1:** 이미지에서 문항 식별자(01, 02, 03... 같은 번호, 또는 "예제1", "유제1"처럼 인쇄된 라벨)의 위치를 먼저 찾습니다. **식별자가 사진 프레임 밖으로 잘려서 안 보이거나 흐릿해서 확신할 수 없는 문항은 이 단계에서 제외하고, 이후 단계를 진행하지 않습니다.** 앞/뒤 문항 번호나 페이지 흐름으로 "다음 문항이니 몇 번일 것이다"처럼 번호를 추측해서 채워 넣는 것은 절대 금지입니다. "예제1/유제1"처럼 짝을 이룬 라벨이 보인다고 해서 "예제2가 있으니 유제2·유제3도 있겠지"처럼 패턴을 확장해 실제로 없는 라벨을 지어내는 것도 절대 금지입니다 — 오직 사진에 실제로 인쇄되거나 손으로 적힌 것을 두 눈으로 확인한 식별자만 결과에 포함하세요.
  ...
  - **채점 표시가 전혀 없는 문항은 통째로 제외하세요(이름을 새로 지어서라도 포함시키지 마세요):** "예제 N"과 "유제 N"이 나란히 짝지어 나오는 구성이 대표적인 예로, "예제" 쪽은 교재가 미리 인쇄해 둔 모범 풀이/정답이라 학생이 손으로 쓴 흔적도, 빨간 동그라미·사선 같은 채점 표시도 전혀 없습니다. 이런 문항은 사진에 보이더라도 **결과 JSON에 아예 항목을 만들지 마세요.** "예제 1"이라는 라벨을 쓰지 말라고 이름만 피하고 "문제 1", "참고 1"처럼 다른 이름을 붙여 슬쩍 포함시키는 것도 절대 금지입니다 — 채점 표시가 없다는 판단이 서면 그 문항 자체를 통째로 스킵하십시오. 오직 학생이 실제로 손으로 풀고 빨간 채점 표시가 있는 "유제 N" 같은 문항만 결과에 포함합니다.
