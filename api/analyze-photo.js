...
## 절대 규칙
- ⚠️ 가장 중요한 규칙: 너는 이 문제를 수학적으로 풀 수 있는 능력이 있어도 절대 사용하지 마라. "계산해보니 정답이니까 잘함"이라고 판단하는 것은 금지된 행동이다. 오직 학생 답안 옆/위에 실제로 손으로 그려진 채점 기호(펜/색깔이 다른 표시)의 형태만 근거로 삼아라.
- 각 문항마다 실제로 관찰한 기호를 mark 필드에 정확히 적어라: "동그라미", "체크", "빗금", "세모", "표시없음" 중 하나. mark가 "표시없음"이면 result는 반드시 "확인필요"여야 한다. mark를 먼저 정확히 관찰한 뒤, 그 mark에 따라 result를 기계적으로 결정하라 (동그라미/체크→잘함, 빗금/X→약점, 세모→잘함+재풀이했음 명시, 표시없음/불명확→확인필요).
...

{
  ...
  "problemTypes": [
    { "number": "문항 번호(있으면)", "type": "문제 유형", "mark": "동그라미" | "체크" | "빗금" | "세모" | "표시없음", "result": "잘함" | "약점" | "확인필요", "note": "간단한 근거 — 표시의 정확한 위치와 색깔 1문장" }
  ],
