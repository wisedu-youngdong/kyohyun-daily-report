...
- 원도 사선도 전혀 없어 채점 여부 자체를 알 수 없는 문항은 "오답"이 아니라 **채점되지 않은 문항**입니다. 이런 문항은 결과에서 완전히 제외하십시오 — "채점 표시 없음"을 이유로 오답 처리하는 것은 명백한 오류입니다.
...
- **Step 1:** 이미지에서 문항 식별자(01, 02, 03... 같은 번호, 또는 "예제1", "유제1"처럼 인쇄된 라벨)의 위치를 먼저 찾습니다. **식별자가 사진 프레임 밖으로 잘려서 안 보이거나 흐릿해서 확신할 수 없는 문항은 이 단계에서 제외하고, 이후 단계를 진행하지 않습니다.** 앞/뒤 문항 번호나 페이지 흐름으로 "다음 문항이니 몇 번일 것이다"처럼 번호를 추측해서 채워 넣는 것은 절대 금지입니다. "예제1/유제1"처럼 짝을 이룬 라벨이 보인다고 해서 "예제2가 있으니 유제2·유제3도 있겠지"처럼 패턴을 확장해 실제로 없는 라벨을 지어내는 것도 절대 금지입니다 — 오직 사진에 실제로 인쇄되거나 손으로 적힌 것을 두 눈으로 확인한 식별자만 결과에 포함하세요.
  ...
  - **채점 표시(빨간 원 또는 사선)가 전혀 없는 문항은 이유를 불문하고 통째로 제외하세요 — 절대로 오답으로 처리하지 마세요:** 이건 "예제 N"/"유제 N" 짝(예제 쪽은 교재가 미리 인쇄한 모범 풀이라 채점 표시가 없음)에만 적용되는 규칙이 아니라, **모든 문항에 예외 없이 적용되는 규칙**입니다. 채점되지 않았거나, 학생이 안 풀었거나, 이 페이지가 아직 채점 전이거나 — 이유가 무엇이든 그 문항 주변에 빨간 원이나 사선이 눈으로 확인되지 않으면 **결과 JSON에 아예 항목을 만들지 마세요.** "문제 1", "참고 1"처럼 다른 이름을 붙여 슬쩍 포함시키는 것도 금지, "표시가 없으니 오답일 것이다"라고 추론해 오답으로 넣는 것도 금지입니다. 오직 학생이 실제로 풀고 빨간 채점 표시가 남아있는 문항만 결과에 포함합니다.
...
- **Step 5 (확신도 표시):** 판정을 내리긴 했지만 아래처럼 표시 자체가 애매했던 문항은 confidence를 "low"로 표시하세요(그 외엔 "high") — 판정을 포기하라는 게 아니라, 가장 그럴듯한 판정은 내리되 사람이 한 번 더 확인해야 함을 표시하는 것입니다.
