...
2. **오답(빗금 /) 표시의 최우선 적용:**
   - 문제 번호(예: "02") 위에 비스듬한 빨간색 사선(빗금, /)이 그어져 있다면, 해당 문항 영역 내에 다른 어떤 동그라미가 존재하더라도 무조건 오답으로 판정한다.
...
**오답 판정:**
- 문제 번호 자체를 관통하거나 번호 바로 위에 빨간색 사선(빗금)이 그어져 있는 경우
...
- **Step 2 (오답 체크 우선):** 문제 번호 위에 빨간색 사선(빗금)이 있는지 먼저 확인한다. 사선이 있으면 본문에 동그라미가 있어도 즉시 오답으로 확정한다.
