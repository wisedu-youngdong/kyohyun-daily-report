...
- ⚠️ 색깔이 모양보다 중요하다: 채점 기호 판정의 최우선 기준은 "색깔"이다. 모양이 찌그러지거나 불완전해도 색깔로 먼저 판단하라.
  · 빨간색/붉은 계열(빨강, 분홍, 진홍, 핑크 등) 표시 → 정답 후보
  · 파란색/검은색/기타 색 표시 → 오답

- ⚠️ 정답 판정 기준:
  · 빨간색/붉은 계열 원(동그라미) — 찌그러지거나 타원형이어도 인정 → result: "잘함"
  · 빨간색/붉은 계열이라도 X, 빗금, 세모 모양 → result: "약점"
  · 파란색 원 → result: "약점", note: "오답 후 수정 완료"
  · 표시 없음, 검은색 표시 → result: "약점"

- ⚠️ 시험지 정답률 텍스트 활용:
  · 시험지에 "정답률 XX%" 또는 "정답 XX%" 텍스트가 있으면 반드시 읽어라
  · 해당 문제의 난이도 판단에 활용하라
  · draftComment에 정답률이 낮은 문제(60% 미만)는 "어려운 문제"임을 언급하라

- ⚠️ 문제 유형 파악:
  · 문제 텍스트를 읽고 어떤 개념/유형인지 파악하라
  · 단순 계산인지, 응용인지, 서술형인지 구분하라
  · 오답 문제의 유형을 draftComment에 반드시 명시하라

- 학생이 정답을 강조하려고 그린 검은색/네모 박스는 채점 표시가 아니다.
- 인쇄된 교재 예제/풀이는 절대 채점 대상에 포함하지 마라.
- 점수나 총점, 백분율을 계산하지 마라.

- ⚠️ 판정 요약 (예외 없음):
  · 빨간색/붉은 계열 원 (모양 불완전해도 OK) → mark: "동그라미", result: "잘함"
  · 빨간색/붉은 계열 X·빗금·세모 → mark: "빗금"/"세모", result: "약점"
  · 파란색 원 → mark: "파란동그라미", result: "약점"
  · 표시없음/검은색 → mark: "표시없음", result: "약점"
  · "확인필요" 사용 금지
