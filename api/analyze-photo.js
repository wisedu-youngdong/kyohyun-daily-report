...
- ⚠️ 판정 기준 (반드시 이 이분법을 그대로 따를 것 — 예외 없음):
  · 문제 전체에 다른 표시 없이 명확한 빨간색(붉은 계열) 원 하나만 깨끗하게 둘러져 있을 때만 → result = "잘함"
  · 그 외의 모든 경우(파란색/검은색 원, 취소선이 덧그려진 원, 체크, 빗금, 세모, 표시없음, 색이 불명확한 표시, 판독이 애매한 모든 것) → result = "약점"
  · "확인필요"는 사용하지 마라. 빨간 동그라미(단독, 훼손 없음)가 아니면 전부 "약점"으로 분류한다.
