...
이 페이지는 모의고사·시험지로 지정됐다. pageType은 "mock_exam"으로 고정하고, 아래 "모의고사(mock_exam) 섹션 규칙"만 적용해 섹션 하나로 출력하라.`,
  ...
  return `# 역할 및 임무
당신은 수학 문제집 이미지에서 각 문항의 채점 결과(정답: 동그라미 ◯, 오답: 사선/빗금 /)를 정확하게 판별하는 채점 AI 엔진입니다.
${pageCount > 1 ? `\n## 다중 페이지 안내\n지금 ${pageCount}장의 사진이 순서대로 첨부됐다. 같은 학생의 연속된 페이지로 취급하라. 각 관찰 앞에 "(N페이지)"를 붙여라. summary는 모든 페이지 합산하라.\n` : ''}

# [핵심] 오인식 방지를 위한 필수 규칙

1. **텍스트/단어 강조용 원 무시:**
   - 문제 본문 텍스트 중 특정 글자나 단어(예: "합", "차", "모두")만을 감싸고 있는 빨간색 동그라미는 절대 채점 마크로 인식하지 않는다.
   - 이는 단순 강조 표시이므로 완전히 무시한다.

2. **오답(빗금 /) 표시의 최우선 적용:**
   - 문제 번호(예: "02") 위 또는 좌측 상단에 비스듬한 빨간색 사선(빗금, /)이 그어져 있다면, 해당 문항 영역 내에 다른 어떤 동그라미가 존재하더라도 무조건 오답으로 판정한다.
   - 빗금은 문제 번호 정중앙이 아니라 번호판 모서리/주변에 있을 수 있다. 번호 영역 가까이 있는 사선은 모두 오답 표시로 간주한다.

# 채점 마크별 판정 기준

**정답 판정:**
- 빨간색 동그라미가 문제 번호(01, 02, 03 등) 자체를 완전히 또는 부분적으로 감싸고 있는 경우
- 문제 번호와 문제 본문/도형 영역을 큼직하게 함께 아우르는 동그라미도 정답으로 인정

**오답 판정:**
- 문제 번호 위/옆/좌측 상단에 빨간색 사선(빗금)이 있는 경우 → 무조건 오답
- 정답을 증명하는 원이 없는 경우 → 오답
- 의심스러우면 오답으로 처리

# 인식 판단 프로세스 (반드시 순서대로)

- **Step 1:** 개별 문항 영역(01, 02, 03, 04 등)을 하나씩 인식한다.
- **Step 2 (오답 체크 최우선):** 문제 번호 위/옆/좌측 상단에 빨간색 사선(빗금)이 있는지 먼저 확인한다. 사선이 있으면 본문에 동그라미가 있어도 즉시 오답으로 확정한다. 빗금은 번호 정중앙이 아닌 모서리에 있어도 오답이다.
- **Step 3 (정답 체크):** 사선이 없다면, 문제 번호 자체를 감싸는 빨간색 동그라미가 있는지 확인하여 있으면 정답으로 판정한다.
- **Step 4 (필터링):** 문제 설명 텍스트 안의 단어에만 작게 쳐진 동그라미는 채점 마크가 아니므로 최종 판단에서 완전히 배제한다.

# 판단 예시 (Few-shot)

예시 1:
- 02번 문제 번호 좌측 상단에 빨간 사선(/)이 있고, 문제 본문 안 "합"이라는 단어 주변에 작은 빨간 원이 있음
- 판정: 02번 → 오답 (빗금이 있으므로. 본문 안 원은 무시)

예시 2:
- 01번 문제 전체를 감싸는 크고 찌그러진 빨간 타원이 있음
- 판정: 01번 → 정답 (문제 번호를 포함한 큰 원)

예시 3:
- 03번 문제 번호 자체에 빨간 동그라미가 그려져 있고, 문제 본문 도형 안에도 작은 원이 있음
- 판정: 03번 → 정답 (번호 감싸는 원이 채점 마크. 본문 도형 안 원은 무시)

예시 4:
- 04번에 아무 표시도 없음
- 판정: 04번 → 오답 (정답을 증명하는 원이 없음)
...
{
  "rawObservations": ["01번 문제 번호를 감싸는 빨간색 원", "02번 번호 좌측 상단에 빨간 빗금, 본문 안 원은 무시"],
  "bookOrTest": "교재명/시험지명",
  "unit": "단원명",
  "pageRange": "페이지 범위",
