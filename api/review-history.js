...
// Firestore 규칙상 reviews의 read/list는 직원 전용(전체 복습 이력을 아무나 못 긁어가게)이라,
// 특정 studentId·완료 건으로 스코프한 결과만 Admin SDK로 대신 조회해서 돌려준다.
// note(강사가 남긴 조치 메모)는 학부모에게 복습 안내 문구로 그대로 보여줄 목적으로 의도적으로
// 포함 — GrowthStory.jsx가 그대로 렌더링함. 나중에 진짜 직원 전용 메모 필드가 추가되면
// 여기서 반드시 빼야 함.
