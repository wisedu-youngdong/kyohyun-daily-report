...
async function generateNarrative(studentName, milestones, unitScores) {
  try {
    const prompt = `
당신은 20년 경력의 교육 전문가입니다. 아래 학생 데이터를 바탕으로 학부모에게 전달할 성장 스토리를 작성하세요.

학생명: ${studentName}
성장 마일스톤:
${milestones.map((m, i) => `PHASE ${i+1}: ${m.date} — ${m.title} (${m.badge})`).join('\n')}
단원별 점수:
${unitScores.map(u => `${u.unit}: ${u.scores.map(s => `${s.round} ${s.score}점`).join(' → ')}`).join('\n')}

다음 형식으로 JSON만 반환하세요 (다른 텍스트 없이):
{
  "chapter1": "성장의 출발점 서사 (2~3문장, 따뜻하고 구체적으로)",
  "chapter2": "결정적 전환점 서사 (2~3문장, 인지적 변화 중심으로)",
  "teacherWord": "선생님 한마디 (1~2문장, 핵심 성장 포인트)",
...
export default function GrowthStory() {
  ...
  const handleGenNarrative = async () => {
    setNarLoading(true);
    const result = await generateNarrative(student?.name || '학생', milestones, unitScores);
    setNarrative(result);
    setNarLoading(false);
  };
        ...
        <div style={{ position: 'relative', paddingLeft: '28px' }}>
          ...
          {milestones.map((m, i) => (
            <div key={i} style={{ position: 'relative', marginBottom: i < milestones.length - 1 ? '20px' : 0 }}>
              ...
              <p style={{ fontSize: '13px', fontWeight: 700, color: '#0D2D6B', marginBottom: '4px' }}>{m.title}</p>
              <p style={{ fontSize: '12px', color: '#4A4A4A', lineHeight: 1.8, wordBreak: 'keep-all', marginBottom: '6px' }}>
                {narrative ? (i === 0 ? narrative.chapter1 : i === milestones.length - 1 ? narrative.chapter2 : m.desc) : m.desc}
              </p>
