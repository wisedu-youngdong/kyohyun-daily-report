        ...
        <div ref={cardRef} style={{ background: DS.paper }}>
          ...
          <div style={{ borderTop: `1px solid ${DS.rule}`, paddingTop: '20px', marginBottom: '4px' }}>
            ...
            <p style={{ fontSize: '12px', color: DS.ink, margin: 0, lineHeight: 2.0, wordBreak: 'keep-all' }}>
              {(() => {
                // 교재명/단원 텍스트에서 순수 단원명만 추출하는 함수
                const extractUnitName = (str) => {
                  if (!str) return '';
                  return str
                    .replace(/디돔돌|디딤돌/g, '')
                    .replace(/기본\+응용|기본\s*\+\s*응용/g, '')
                    .replace(/[0-9]+[-–][0-9]+/g, '')  // 6-2 같은 학년 제거
                    .replace(/·/g, '')
                    .replace(/\s+/g, ' ')
                    .trim();
                };
                ...
                // 교재 base명 (첫 문장 1회만)
                const mainTextbook = sorted.map(r => r.textbook).filter(Boolean)[0] || '';
                const fixedTextbook = mainTextbook.replace(/디돔돌/g, '디딤돌');
                const textbookBase = fixedTextbook.replace(/\s*·?\s*\d+단원.*$/, '').trim();

                // 단원명 목록 (교재명 제거 후 순수 단원명만)
                const rawUnits = [...new Set(sorted.map(r => r.unit).filter(Boolean))];
                const cleanUnits = rawUnits.map(extractUnitName).filter(u => u.length > 0);
                const uniqueUnits = [...new Set(cleanUnits)].slice(0, 3);
                ...
                // part2 — 최고 이해도 (단원명만, 교재명 없음)
                const bestUnitClean = extractUnitName(unitOf(bestConceptReport));
                const part2 = bestConceptReport
                  ? ` 특히 ${fmtShort(bestConceptReport)} 수업${bestUnitClean ? `(${bestUnitClean})` : ''}에서는 개념을 스펀지처럼 흡수하며 기간 내 가장 높은 이해도를 보여주었습니다. 학습 리듬을 타기 시작했다는 긍정적인 신호입니다.`
                  : '';

                // part3 — 보완 (단원명만 1회, 마지막 문장에 반복 없음)
                const weakUnitClean = topTag ? extractUnitName(unitOf(tagFirstExample[topTag[0]])) : '';
                const wUnit = weakUnitClean || '해당 단원';
                const DIAG_FEEDBACK = {
                  calc:    `다만 ${wUnit} 응용 문항에서 풀이 과정을 손으로 정리하는 습관이 아직 정착되지 않아 연산 처리 단계에서 실수가 반복되고 있습니다. 속도보다 정확성을 먼저 확보하는 훈련을 집중적으로 진행하겠습니다.`,
                  concept: `다만 ${wUnit} 학습에서 공식을 암기하는 수준을 넘어 개념이 왜 성립하는지 근거를 스스로 설명할 수 있는 내면화가 아직 진행 중입니다. 개념 재정립 후 적용 단계로 이어지는 흐름을 집중 훈련하겠습니다.`,
                  apply:   `다만 ${wUnit} 응용 문항을 다룰 때, 문제 상황은 잘 인지하면서도 풀이의 판단 기준을 스스로 세우는 과정에서 주춤하는 모습이 관찰되었습니다. 단순한 공식 암기를 지양하고, 아이 스스로 왜 이 방법을 써야 하는지 해결 전략을 도출할 수 있도록 밀착 복습을 진행하겠습니다.`,
                  time:    `다만 ${wUnit} 문항에서 어떤 문제를 먼저 풀어야 하는지 판단하는 전략이 아직 형성되지 않아 시간 배분에 어려움이 관찰되었습니다. 풀이 우선순위 훈련을 체계적으로 진행하겠습니다.`,
                };
                const part3 = topTag
                  ? ` ${DIAG_FEEDBACK[topTag[0]] || `다만 '${TAG_LABELS_LOCAL[topTag[0]]}' 패턴이 반복 관찰되어 ${wUnit} 관련 복습이 필요합니다.`}`
                  : '';
